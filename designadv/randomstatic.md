---
abstract: >
  Use the basic/static randomization features to assign
  patients to research groups,
  in order to increase the internal validity of a study.
  Static (as opposed to dynamic) randomization accommodates...

author:
  - name: Steven Fowler
    affiliation: University of Oklahoma Health Campus
   #  affiliation-url: https://ouhsc.edu/dbp/
   #  email: sssss@ou.edu
    orcid: 0000-0000-0000-0000
    attributes:
      corresponding: true

  - name: Thomas Wilson
    affiliation: University of Oklahoma Health Campus
    affiliation-url: https://ouhsc.edu/bbmc/
    email: thomas-wilson@ou.edu
    orcid: 0009-0009-1239-1348

csl: ../assets/csl/apa-7e.csl
---

# Static Randomization of Patients {#sec-designadv-randomstatic}

**Chapter Leads**: Steven Fowler

::: {.callout-warning appearance="simple"}

## Chapter Draft

This chapter is being written currently,
as is available during development in case the preliminary version is helpful to someone.

If you have suggested modifications or additions, please see [How to Contribute](../index.qmd#sec-welcome-contribute) on the book's welcome page.
:::

## What is the Purpose of Randomization? {#sec-designadv-randomstatic-purpose}

To learn how to request a new project, see [Best Way to Learn](../designbas/variable.md#sec-designbas-variable-howtolearn).

## Preparing to Build the e-Consent

![This is an example screenshot](images/randomstatic/placeholder-1.png){width="80%"}

![Here is another example](images/randomstatic/placeholder-2.png){width="80%"}

::: {.callout-note appearance="simple"}

## Additional Chapter Details

This chapter was last edited in April 2026.
If you have suggested modifications or additions, please see [How to Contribute](../index.qmd#sec-welcome-contribute) on the book's welcome page.
:::
